    ...
    <div className="min-h-screen bg-wfx-bg text-wfx-text font-sans transition-colors pb-0 flex flex-col">
      ...
      <div className="sticky top-0 z-50 w-full flex flex-col shrink-0">
        ...
        {!isPromoDismissed && mounted && (
          <div className="bg-wfx-card/95 backdrop-blur-md border-b border-wfx-border flex items-center justify-center relative py-2 px-3 md:px-4 overflow-hidden animate-in slide-in-from-top-full duration-500">
            ...
            <div className="text-[9px] md:text-xs text-wfx-muted font-medium relative z-10 flex flex-row items-center justify-center gap-1.5 md:gap-2.5 max-w-[88%] md:max-w-none text-center md:text-left leading-tight">
              <span className="flex h-2 w-2 relative shrink-0">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="flex flex-col md:flex-row md:items-center">
                <strong className="text-emerald-500 uppercase tracking-widest md:mr-1">Leve 4, Pague 3:</strong>
                <span>Adicione 4 arquivos no carrinho e o de menor valor sai <strong className="text-wfx-text font-black">totalmente de graça.</strong></span>
              </span>
            </div>

            <button 
              onClick={handleDismissPromo} 
              className="absolute right-2 md:right-6 text-wfx-muted hover:text-wfx-primary transition-colors z-10 p-1.5 rounded-full hover:bg-wfx-bg"
        ...
        <header className="bg-wfx-bg/80 backdrop-blur-md h-16 md:h-20 border-b border-wfx-border relative">
          <div className="max-w-7xl mx-auto px-3 md:px-6 h-full flex items-center justify-between">
            <div className="flex items-center cursor-pointer shrink-0" onClick={scrollToTop}>
              <Image src="/logo.png" alt="WFX Logo" width={90} height={36} priority className="object-contain w-[75px] md:w-[100px]" />
            </div>
            ...
            <div className="flex items-center gap-1 md:gap-3 shrink-0">
              <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="p-1.5 md:p-2 rounded-full hover:bg-wfx-card transition-all text-wfx-muted hover:text-wfx-primary">
                {theme === "dark" ? <Moon className="w-[18px] h-[18px] md:w-5 md:h-5" /> : <Sun className="w-[18px] h-[18px] md:w-5 md:h-5" />}
              </button>
              <Link href="/perfil" prefetch={false} className="p-1.5 md:p-2 rounded-full hover:bg-wfx-card transition-all text-wfx-muted hover:text-wfx-primary" title="Minha Conta">
                <User className="w-[18px] h-[18px] md:w-5 md:h-5" />
              </Link>
              
              {/* --- BOTÃO DO CARRINHO (IGUAL AO DA PÁGINA PRODUTO) --- */}
              <button onClick={toggleCart} className="flex items-center gap-1.5 md:gap-2 px-2.5 py-1.5 md:px-3 md:py-2 bg-wfx-primary text-white hover:opacity-90 transition-all text-[10px] md:text-sm font-bold uppercase tracking-wide rounded-sm shadow-lg shadow-blue-500/20 ml-1 md:ml-0">
      ...
      <main>
        {/* --- LANDING PAGE ORIGINAL (SEM FLEX/MARGIN ESTRANHO) --- */}
          ...
          <div className="flex flex-col md:flex-row gap-8 md:gap-12">
            ...
            <aside className="w-full md:w-64 space-y-8 flex-shrink-0 text-wfx-muted">
              ...
              <div>
                ...
                <ul className="grid grid-cols-2 gap-x-4 gap-y-3 md:flex md:flex-col font-medium">
                  {categoriesList.map((item) => (
                    <li key={item} onClick={() => handleCategoryChange(item)} className={`flex items-center gap-3 group cursor-pointer transition-colors ${selectedCategory === item ? 'text-wfx-primary font-bold' : 'hover:text-wfx-primary'}`}>
                      <div className={`w-4 h-4 shrink-0 border rounded-sm flex items-center justify-center transition-all ${selectedCategory === item ? 'border-wfx-primary bg-wfx-primary' : 'border-wfx-border group-hover:border-wfx-primary'}`}>{selectedCategory === item && <div className="w-2 h-2 bg-white rounded-full"></div>}</div>
              ...
              <div>
                ...
                <ul className="grid grid-cols-2 gap-x-4 gap-y-3 md:flex md:flex-col font-medium">
                  <li onClick={() => handleUsageChange('Prototipagem')} className={`flex items-center gap-3 group cursor-pointer transition-colors ${selectedUsage === 'Prototipagem' ? 'text-wfx-primary font-bold' : 'hover:text-wfx-primary'}`}>
                    <div className={`w-4 h-4 shrink-0 border rounded-sm flex items-center justify-center transition-all ${selectedUsage === 'Prototipagem' ? 'border-wfx-primary bg-wfx-primary' : 'border-wfx-border group-hover:border-wfx-primary'}`}>{selectedUsage === 'Prototipagem' && <div className="w-2 h-2 bg-white rounded-full"></div>}</div>
                    <span className="flex items-center gap-2 text-sm"><span className="w-2 h-2 rounded-full bg-amber-500 shrink-0"></span> Fun. Direta</span>
                  </li>
                  <li onClick={() => handleUsageChange('Borracha')} className={`flex items-center gap-3 group cursor-pointer transition-colors ${selectedUsage === 'Borracha' ? 'text-wfx-primary font-bold' : 'hover:text-wfx-primary'}`}>
                    <div className={`w-4 h-4 shrink-0 border rounded-sm flex items-center justify-center transition-all ${selectedUsage === 'Borracha' ? 'border-wfx-primary bg-wfx-primary' : 'border-wfx-border group-hover:border-wfx-primary'}`}>{selectedUsage === 'Borracha' && <div className="w-2 h-2 bg-white rounded-full"></div>}</div>
                    <span className="flex items-center gap-2 text-sm"><span className="w-2 h-2 rounded-full bg-slate-400 shrink-0"></span> Molde de Borracha</span>
                  </li>
                </ul>
              </div>
            </aside>
            ...
            <div className="flex-1 flex flex-col min-h-[600px]">
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-6">
                  {products.map((product) => (
                    <Link href={`/produto/${product.id}`} key={product.id} className="group border border-wfx-border hover:border-wfx-primary bg-wfx-card p-3 md:p-4 transition-all duration-300 hover:shadow-xl cursor-pointer block flex flex-col h-full">
                      <div className="aspect-[4/3] bg-wfx-bg mb-4 flex items-center justify-center relative overflow-hidden rounded-sm border border-wfx-border/50 shrink-0">
                        ...
                        {product.usage === 'Prototipagem' && <span className="absolute top-2 right-2 bg-amber-100 text-amber-800 text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 md:px-2 md:py-1 uppercase border border-amber-200 shadow-sm">Fun. Direta</span>}
                        {product.usage === 'Borracha' && <span className="absolute top-2 right-2 bg-slate-200 text-slate-700 text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 md:px-2 md:py-1 uppercase border border-slate-300 shadow-sm">Molde Borracha</span>}
                      </div>
                      
                      <div className="flex flex-col flex-1 justify-between gap-3">
                        {/* --- FONTE AUMENTADA NO MOBILE (text-[14px]) COM ALTURA ESTÁTICA FIXA (h-[36px]) --- */}
                        <h3 className="font-bold text-[13.8px] sm:text-[14px] md:text-[15px] leading-snug group-hover:text-wfx-primary transition-colors text-wfx-text line-clamp-2 h-[38px] sm:h-[40px] md:h-[44px]">{product.title}</h3>
                        <div className="flex flex-col pt-2 border-t border-wfx-border gap-0.5 mt-auto">
                          <span className="text-[9px] md:text-[10px] lg:text-xs text-wfx-muted uppercase truncate">{product.category}</span>
                          <span className="font-black text-[15px] sm:text-base md:text-lg lg:text-xl text-wfx-primary tracking-tighter">
              ...
              {!loading && totalFiltered > ITEMS_PER_PAGE && (
                /* --- PAGINAÇÃO (ALINHADA NA MESMA LINHA NO MOBILE) --- */
                <div className="flex justify-center items-center gap-3 md:gap-4 mt-12 border-t border-wfx-border pt-8">
                  <button 
                    onClick={() => handlePageChange(Math.max(1, page - 1))} 
                    disabled={page === 1} 
                    className="p-2 md:px-4 md:py-2 border border-wfx-border rounded-md hover:border-wfx-primary disabled:opacity-30 disabled:hover:border-wfx-border transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-wider"
                  >
                    <ChevronLeft size={20} /> <span className="hidden md:inline">Anterior</span>
                  </button>
                  
                  <span className="font-mono text-[11px] md:text-sm text-wfx-muted px-2 md:px-4">
                    Página <span className="text-wfx-text font-bold text-sm md:text-base">{page}</span> de {Math.ceil(totalFiltered / ITEMS_PER_PAGE)}
                  </span>
                  
                  <button 
                    onClick={() => handlePageChange(page + 1)} 
                    disabled={page * ITEMS_PER_PAGE >= totalFiltered} 
                    className="p-2 md:px-4 md:py-2 border border-wfx-border rounded-md hover:border-wfx-primary disabled:opacity-30 disabled:hover:border-wfx-border transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-wider"
                  >
                    <span className="hidden md:inline">Próxima</span> <ChevronRight size={20} />
                  </button>
                </div>
              )}
            </div>
