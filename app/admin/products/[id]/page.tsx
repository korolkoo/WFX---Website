...
export default function EditProductPage() {
    ...
    }>({ image: null, stl: null, zip: null, glb: null, video360: null, videoReal: null });
    ...
    const handleGlbSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            setGlbPreviewUrl(URL.createObjectURL(e.target.files[0]));
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (imageFile) { finalUrls.image = await uploadFile('images', imageFile); newlyUploadedUrls.push(finalUrls.image); }
            if (glbFile) { finalUrls.glb = await uploadFile('models', glbFile); newlyUploadedUrls.push(finalUrls.glb); }
            if (video360File) { finalUrls.video360 = await uploadFile('videos', video360File); newlyUploadedUrls.push(finalUrls.video360); }
            if (videoRealFile) { finalUrls.videoReal = await uploadFile('videos', videoRealFile); newlyUploadedUrls.push(finalUrls.videoReal); }
            ...
            if (deliveryFile) {
                ...
                if (fileName.endsWith('.zip') || fileName.endsWith('.rar')) {
                    finalUrls.zip = uploadedUrl;
                    finalUrls.stl = null;
                } else {
                    finalUrls.stl = uploadedUrl;
                    finalUrls.zip = null;
                }
            } else if (externalLink && externalLink !== existingUrls.zip) {
                finalUrls.zip = externalLink;
                finalUrls.stl = null; 
            } else if (!deliveryFile && !externalLink) {
                 finalUrls.zip = null;
                 finalUrls.stl = null;
            }
            ...
            if (hasStoneData) {
                stonesSummary = stoneRows.filter(row => row.qty || row.name).map(row => `${row.qty || '?'} un. ${row.name || 'Pedra'} (Total: ${row.weight ? `${row.weight}g` : '0g'})`).join(' + ');
            }

            const updates = {
                title: formData.title, category: formData.category, price: parseFloat(formData.price),
                usage: formData.usage, description: formData.description, size: formData.size,
                volume: formData.volume ? parseFloat(formData.volume) : null,
                image_url: finalUrls.image, file_url: finalUrls.stl, zip_url: finalUrls.zip, glb_url: finalUrls.glb,
                video_360_url: finalUrls.video360, video_real_url: finalUrls.videoReal,
                material_config: materialConfig, stones_info: stonesSummary
            };
            ...
            Promise.all([
                ...
                deleteOldFile(oldUrls.video360, finalUrls.video360, 'videos'), deleteOldFile(oldUrls.videoReal, finalUrls.videoReal, 'videos')
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 mb-8 items-stretch">
                        <div className="flex flex-col h-full">
                            ...
                            <div className={`relative border-2 border-dashed rounded-lg p-6 text-center group cursor-pointer transition-colors flex-1 flex flex-col items-center justify-center ${glbFile ? 'border-blue-500 bg-blue-900/10' : 'border-slate-700 hover:border-blue-500 bg-slate-950'}`}>
                                ...
                                <span className="text-xs font-medium truncate w-full px-2 text-slate-500 group-hover:text-blue-400">
                        ...
                        <div className="flex flex-col h-full">
                            <label className="text-xs font-bold uppercase text-slate-500 mb-2 shrink-0 flex justify-between">
                                Arquivo Final
                                {(existingUrls.stl || existingUrls.zip) && <span className="text-green-500 text-[10px] ml-2">(JÁ POSSUI)</span>}
                            </label>
                            ...
                            <div className="flex-1 flex flex-col justify-between bg-slate-950 border border-slate-800 p-4 rounded-lg">
                                ...
                                <div className={`relative shrink-0 ${deliveryFile ? 'opacity-30 pointer-events-none' : ''}`}>
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-slate-900 border border-slate-700 rounded-lg py-2.5 pl-9 pr-3 text-xs text-white focus:border-blue-500 outline-none placeholder:text-slate-600"
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <div className={`relative border-2 border-dashed rounded-lg p-6 text-center group cursor-pointer transition-colors flex-1 flex flex-col items-center justify-center ${imageFile ? 'border-green-500 bg-green-900/10' : 'border-slate-700 hover:border-green-500 bg-slate-950'}`} style={!imageFile && existingUrls.image ? { backgroundImage: `url(${existingUrls.image})`, backgroundSize: 'cover', backgroundPosition: 'center' } : {}}>
                                ...
                                <span className="text-xs font-medium truncate w-full px-2 drop-shadow-md text-slate-500 group-hover:text-green-400 relative z-20">
                                    {imageFile ? imageFile.name : (existingUrls.image ? "Trocar Imagem" : "Enviar Imagem")}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-slate-800">
                        <div className="space-y-2"><label className="text-xs font-bold uppercase text-slate-500 flex justify-between"><div className="flex items-center gap-2"><Video size={14} /> Vídeo 360°</div>{existingUrls.video360 && <span className="text-green-500 text-[10px]">(JÁ POSSUI)</span>}</label><div className={`relative border-2 border-dashed rounded-lg p-4 text-center group cursor-pointer transition-colors ${video360File ? 'border-purple-500 bg-purple-900/10' : 'border-slate-700 hover:border-purple-500 bg-slate-950'}`}><input type="file" accept="video/*" onChange={(e) => { if(e.target.files && e.target.files[0]) setVideo360File(e.target.files[0]) }} className="absolute inset-0 opacity-0 cursor-pointer" /><div className="flex items-center justify-center gap-2 text-slate-500 group-hover:text-purple-400"><Film size={18} /><span className="text-xs font-medium truncate">{video360File ? video360File.name : (existingUrls.video360 ? "Trocar Vídeo 360" : "Selecionar Vídeo")}</span></div></div></div>
                        <div className="space-y-2"><label className="text-xs font-bold uppercase text-slate-500 flex justify-between"><div className="flex items-center gap-2"><Video size={14} /> Vídeo Real</div>{existingUrls.videoReal && <span className="text-green-500 text-[10px]">(JÁ POSSUI)</span>}</label><div className={`relative border-2 border-dashed rounded-lg p-4 text-center group cursor-pointer transition-colors ${videoRealFile ? 'border-pink-500 bg-pink-900/10' : 'border-slate-700 hover:border-pink-500 bg-slate-950'}`}><input type="file" accept="video/*" onChange={(e) => { if(e.target.files && e.target.files[0]) setVideoRealFile(e.target.files[0]) }} className="absolute inset-0 opacity-0 cursor-pointer" /><div className="flex items-center justify-center gap-2 text-slate-500 group-hover:text-pink-400"><Film size={18} /><span className="text-xs font-medium truncate">{videoRealFile ? videoRealFile.name : (existingUrls.videoReal ? "Trocar Vídeo Real" : "Selecionar Vídeo")}</span></div></div></div>
                    </div>

                    {glbPreviewUrl && (
                        <div className="mt-8"><div className="mb-2 flex items-center gap-2"><h3 className="text-sm font-bold text-blue-400 uppercase">Configuração de Cores</h3></div><ModelConfigurator fileUrl={glbPreviewUrl} initialConfig={materialConfig} onConfigChange={setMaterialConfig} /></div>
                    )}
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-8">
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl space-y-6">
                        ...
                        <div><label className="block text-sm font-medium text-slate-400 mb-1">Finalidade</label><div className="flex gap-4"><label className="flex items-center gap-2 cursor-pointer bg-slate-950 p-3 rounded-lg border border-slate-800 hover:border-blue-500 flex-1 transition-all"><input type="radio" name="usage" value="Prototipagem" checked={formData.usage === 'Prototipagem'} onChange={e => setFormData({ ...formData, usage: e.target.value as any })} className="text-blue-500 focus:ring-0" /><span className="text-sm text-white">Prototipagem</span></label><label className="flex items-center gap-2 cursor-pointer bg-slate-950 p-3 rounded-lg border border-slate-800 hover:border-blue-500 flex-1 transition-all"><input type="radio" name="usage" value="Borracha" checked={formData.usage === 'Borracha'} onChange={e => setFormData({ ...formData, usage: e.target.value as any })} className="text-blue-500 focus:ring-0" /><span className="text-sm text-white">Molde Borracha</span></label></div></div>
                    </div>

                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl space-y-6">
                        <div className="flex justify-between items-start"><h2 className="text-xl font-bold flex items-center gap-2"><Calculator className="text-amber-500" /> Calculadora de Peso</h2><div className="text-[10px] text-amber-500 bg-amber-900/20 px-2 py-1 rounded border border-amber-900">Atenção: Recalcule se houve mudanças</div></div>
                        <div className="bg-slate-950 p-4 rounded-lg border border-slate-800"><label className="text-sm font-bold text-blue-400 mb-2 block flex items-center gap-2"><Scale size={16} /> Volume do 3D (cm³)</label><input type="number" step="0.001" value={formData.volume} onChange={e => setFormData({ ...formData, volume: e.target.value })} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white font-mono text-lg placeholder:text-slate-600" placeholder="Ex: 0.142" /><p className="text-[10px] text-slate-500 mt-1">Insira o volume em cm³.</p></div>
                        <div className="bg-slate-950 p-4 rounded-lg border border-slate-800"><label className="text-sm font-bold text-purple-400 mb-4 block flex items-center gap-2"><Gem size={16} /> Pedras (Soma Direta no Peso Final)</label>
                            <div className="flex flex-col gap-3">
                                {stoneRows.map((row, index) => (
                                    <div key={index} className="grid grid-cols-12 gap-2 items-end">
                                        ...
                                        <div className="col-span-1 flex justify-center pb-2"><button type="button" onClick={() => removeStoneRow(index)} className="text-slate-600 hover:text-red-500"><Trash2 size={16} /></button></div>
                                    </div>
                                ))}
                            </div>
                            <button type="button" onClick={addStoneRow} className="mt-4 flex items-center gap-2 text-xs font-bold text-blue-400 hover:text-blue-300 transition-colors"><Plus size={14} /> Adicionar Pedra</button>
                            {calculationData.stonesTotal > 0 && (<div className="mt-3 text-right text-xs text-slate-400 border-t border-slate-800 pt-2">Peso Somado das Pedras: <span className="text-white font-mono font-bold">{calculationData.stonesTotal.toFixed(3)}g</span></div>)}
                        </div>
                        <div className="space-y-3 pt-2">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="bg-slate-800/50 p-3 rounded border border-slate-600/50 flex justify-between items-center"><span className="text-xs text-slate-400 font-bold uppercase">Prata 925</span><span className="font-mono font-bold text-white">{calculationData.silver.toFixed(2)} g</span></div>
